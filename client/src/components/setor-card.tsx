...
export function SetorCard({ setor, compact }: SetorCardProps) {
  const { adminOpen, requireAdmin } = useAdmin();
  return (
    <Card className={`hover:shadow-lg transition-all duration-200 flex flex-col ${compact ? "min-h-[200px]" : "min-h-[280px]"}`} data-testid={`card-setor-${setor.id}`}>
      <CardHeader className="p-4 pb-2">
        <div className="flex items-start justify-between gap-2 mb-2">
          ...
          <h3 className={`font-semibold leading-tight line-clamp-2 break-words ${compact ? "min-h-[2.25rem] text-sm md:text-base" : "min-h-[3rem]"} ${setor.nome.length > 42 ? "text-sm md:text-base" : "text-base md:text-lg"}`} data-testid={`text-nome-${setor.id}`}>
            {setor.nome}
          </h3>
        </div>
      ...
      <CardContent className={`flex-1 ${compact ? "space-y-2" : "space-y-3"} p-4 pt-2`}>
        ...
        {(!compact) && setor.responsaveis && setor.responsaveis.some(r => String(r.nome || "").trim().length > 0) && (
